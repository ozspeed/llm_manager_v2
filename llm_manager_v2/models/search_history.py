"""
search_history.py (v2)

Manages recent Hugging Face search queries and trending/cached model queries for the LLM Model Manager (v2).
...
def get_max_history_limit() -> int:
    ...
    try:
        config = get_config()
        return int(getattr(config, 'MAX_SEARCH_HISTORY', 20))
    except Exception:
        return 20
...
def get_trending_model_type() -> str:
    ...
    config = get_config()
    return getattr(config, 'TRENDING_MODEL_TYPE', 'gguf')
